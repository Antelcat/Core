...
public class Tests
{
    ...
    public void TestResolveNativeCollection()
    {
        ...
        Assert.That(((A)aS.ElementAt(0)).B, Is.Null);
    }
    [Test]
    ...
    [Test]
    public void TestNativeGeneric()
    {
        var generic = nativeProvider.GetService<IMultiGeneric<int,double,object>>();
        var times = Times;
        var watch = new Stopwatch();
        watch.Start();
        while (times > 0)
        ...
        watch.Stop();
        Console.WriteLine($"Native generic cost {watch.ElapsedTicks}");
    ...
    public void TestAutowiredGeneric()
    {
        ...
        watch.Stop();
        Console.WriteLine($"Autowired generic cost {watch.ElapsedTicks}");
        Assert.That(generic, Is.Not.Null);
    }
}